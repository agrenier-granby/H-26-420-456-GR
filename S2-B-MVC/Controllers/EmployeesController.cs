...
using S2_B_MVC.Models;
using S2_B_MVC.ViewModels;
...
namespace S2_B_MVC.Controllers
{
    public class EmployeesController : Controller
    {
        ...
        private static IList<Employee> GenerateEmployees()
        {
            return
            [
                new Employee() { Id = 1, Name = "Antoine", HiringDate = DateTimeOffset.Parse("2016-11-23"), YearlySalary = 2000000, Country = Countries[2] },
        ...
        {
            return string.Join("", employees.Select(e => e.ToString()));
        }
        ...
        private PasserInfoVM GetPasserInfoVM()
        {
            var employees = GenerateEmployees();
            var vm = new PasserInfoVM();

            var groupedByCountry = employees
                .GroupBy(e => e.Country?.Name ?? "Pays Inconnu")
                .OrderByDescending(g => g.Count())
                .ToList();

            vm.CountriesInfo = groupedByCountry
                .Select(g => new CountryInfo { CountryName = g.Key, EmployeeCount = g.Count() })
                .ToList();
